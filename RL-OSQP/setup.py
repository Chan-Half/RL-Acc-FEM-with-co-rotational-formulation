...
def get_torch_library_dir():
    import torch
    print(os.path.join(os.path.dirname(os.path.dirname(torch.utils.cmake_prefix_path)), "lib"))
...
rlqp_codegen_sources_dir = os.path.join('rlqp', 'codegen', 'sources')
...
class build_ext_rlqp(build_ext):
    def build_extensions(self):
        ...
        copyfile(lib_origin, os.path.join('rlqp', lib_name))
...
_rlqp = Extension(
    ...
    extra_compile_args=compile_args,
    extra_link_args=[
        f"-Wl,-rpath,{os.path.join(os.path.dirname(os.path.dirname(torch.utils.cmake_prefix_path)), 'lib')}"
    ]
)
...
setup(name='rlqp',
      ...
      package_dir={'rlqp': 'rlqp',
      'rlqp.codegen': 'rlqp/codegen',
      'rlqp.tests': 'rlqp/tests',
