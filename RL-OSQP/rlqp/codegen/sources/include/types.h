...
# if EMBEDDED != 1
  ...
  enum rlqp_adaptive_sigma_type adaptive_sigma; ///< rho adaptation mode/policy (boolean compatible, 0=diable, 1=OSQP's adaptation policy, 2=scalar policy, 3=vector policy)
...
#  endif // Profiling
  ...
  const char *adaptive_sigma_policy; ///< Trained RL policy to use 
# endif // EMBEDDED != 1
...
# endif // if EMBEDDED != 1
  ...
  void *rl_sigma_policy; ///< RL policy for adapting sigma (pointer to a C++ type)
...
# if EMBEDDED != 1
  ...
  c_int (*update_sigma_vec)(LinSysSolver  *s,
                          const c_float *sigma_vec);  ///< Update sigma_vec
# endif // if EMBEDDED != 1
