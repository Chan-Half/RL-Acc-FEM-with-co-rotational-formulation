...
void free_linsys_solver_qdldl(qdldl_solver *s) {
    if (s) {
        ...
        if (s->sigma_vec)   c_free(s->sigma_vec);
        ...
        if (s->sigmatoKKT)  c_free(s->sigmatoKKT);
...
static c_int permute_KKT(csc ** KKT, qdldl_solver * p, c_int Pnz, c_int Anz, c_int n, c_int m, c_int * PtoKKT, c_int * AtoKKT, c_int * rhotoKKT, c_int * sigmatoKKT){
    c_float *info;
    c_int amd_status;
    c_int * Pinv;
    csc *KKT_temp;
    ...
    // Permute KKT matrix
    if (!PtoKKT && !AtoKKT && !rhotoKKT && !sigmatoKKT){  // No vectors to be stored
        ...
        if (sigmatoKKT){
            for (i = 0; i < n; i++){
                sigmatoKKT[i] = KtoPKPt[sigmatoKKT[i]];
            }
        }
...
c_int init_linsys_solver_qdldl(qdldl_solver ** sp, const csc * P, const csc * A, c_float *sigma_vec, const c_float * rho_vec, c_int polish){
    ...
    *sp = s;
    ...
    s->update_sigma_vec = &update_linsys_solver_sigma_vec_qdldl;
#endif
    ...
    s->sigma_vec = (c_float *)c_malloc(sizeof(c_float) * s->n);
    ...
    if (polish){ // Called from polish()
        ...
        KKT_temp = form_KKT(P, A, 0, s->sigma_vec, s->rho_inv_vec, OSQP_NULL, OSQP_NULL, OSQP_NULL, OSQP_NULL, OSQP_NULL, OSQP_NULL);

        // Permute matrix
        if (KKT_temp)
            permute_KKT(&KKT_temp, s, OSQP_NULL, OSQP_NULL, OSQP_NULL, OSQP_NULL, OSQP_NULL, OSQP_NULL, OSQP_NULL, OSQP_NULL);
    }
        ...
        s->sigmatoKKT = c_malloc((s->n) * sizeof(c_int));
        ...
        for (i = 0; i < s->n; i++){
            s->sigma_vec[i] = sigma_vec[i];
        }

        KKT_temp = form_KKT(P, A, 0, s->sigma_vec, s->rho_inv_vec,
                            s->PtoKKT, s->AtoKKT,
                            &(s->Pdiag_idx), &(s->Pdiag_n), s->rhotoKKT, s->sigmatoKKT);

        // Permute matrix
        if (KKT_temp)
            permute_KKT(&KKT_temp, s, P->p[P->n], A->p[A->n], s->n, A->m, s->PtoKKT, s->AtoKKT, s->rhotoKKT, s->sigmatoKKT);
    }
...
c_int update_linsys_solver_matrices_qdldl(qdldl_solver * s, const csc *P, const csc *A) {
    ...
    update_KKT_P(s->KKT, P, s->PtoKKT, s->sigma_vec, s->Pdiag_idx, s->Pdiag_n);
...
c_int update_linsys_solver_sigma_vec_qdldl(qdldl_solver * s, const c_float *sigma_vec){
    
    c_int i;
    // Update KKT matrix with new P
    
    update_KKT_sigma_vec(s->KKT, s->sigmatoKKT, sigma_vec, s->sigma_vec, s->n);
    // Update internal sigma_vec
    for (i = 0; i < s->n; i++){
        s->sigma_vec[i] = sigma_vec[i];
    }
