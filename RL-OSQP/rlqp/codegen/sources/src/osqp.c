...
void osqp_set_default_settings(OSQPSettings *settings) {
  ...
  settings->adaptive_sigma           = ADAPTIVE_SIGMA;
...
# endif /* ifdef PROFILING */
  ...
  settings->adaptive_sigma_policy = (char *)OSQP_NULL;
#endif  /* if EMBEDDED != 1 */
...
# endif /* ifdef PROFILING */
  ...
  work->P_diag = csc_diagonal(data->P);
  ...
  work->sigma_vec     = c_malloc(data->n * sizeof(c_float));
  ...
  if (settings->adaptive_sigma_policy != OSQP_NULL) {
    // strdup uses malloc under the hood, which might be a problem
    // if c_malloc is not malloc.
    work->settings->adaptive_sigma_policy = c_strdup(settings->adaptive_sigma_policy);
    if (!(work->settings->adaptive_sigma_policy))
      return osqp_error(OSQP_MEM_ALLOC_ERROR);
  }
...
# ifdef PROFILING
  ...
  work->rl_sigma_policy = rl_policy_load_sigma(settings->adaptive_sigma_policy);
...
# ifdef PROFILING
    ...
    if ((work->settings->adaptive_rho != RLQP_ADAPTIVE_RHO_DISABLE || work->settings->adaptive_sigma != RLQP_ADAPTIVE_SIGMA_DISABLE) && !work->settings->adaptive_rho_interval) {
      // Check time
...
# endif /* ifdef PRINTING */
      // c_print("the adaptive_rho_interval is (%d)\n", work->settings->adaptive_rho_interval);
      work->settings->adaptive_rho_interval = 200;
      // Actually update rho
      if (adapt_rho(work)) {
# ifdef PRINTING
...
# endif // PRINTING
    ...
    // Adapt sigma
    if (work->settings->adaptive_sigma != RLQP_ADAPTIVE_SIGMA_DISABLE &&
        work->settings->adaptive_rho_interval &&
        (iter % work->settings->adaptive_rho_interval == 0)) {
      // Update info with the residuals if it hasn't been done before
# ifdef PRINTING
...
# endif // PRINTING
  ...
  // Compute objective value in case it was 
 ...
 if (work->info->status_val == OSQP_TIME_LIMIT_REACHED) {
  ...
  // Indicate that the solve function has already been executed and executed and excuted
...
c_int osqp_cleanup(OSQPWorkspace *work) {
  ...
  if (work) { // If workspace has been allocated
    ...
    if (work->sigma_vec)     c_free(work->sigma_vec);
    ...
    if (work->settings) {
      ...
      if (work->settings->adaptive_sigma_policy)
        c_free((char *)work->settings->adaptive_sigma_policy);
    ...
    if (work->rl_sigma_policy) rl_policy_unload(work->rl_sigma_policy);
...
c_int osqp_update_sigma_vec(OSQPWorkspace *work, c_float *sigma_vec_new) {
  int exitflag, i;
  // Check if workspace has been initialized
  if (!work)
    return osqp_error(OSQP_WORKSPACE_NOT_INIT_ERROR);
...
# ifdef PRINTING
  ...
  prea_vec_copy(sigma_vec_new, work->sigma_vec, work->data->n);
  work->settings->adaptive_sigma = RLQP_ADAPTIVE_SIGMA_DISABLE;

  for (i=0 ; i<work->data->n ; ++i) {
    work->sigma_vec[i] = c_min(c_max(work->sigma_vec[i], SIGMA_VEC_MIN), SIGMA_VEC_MAX);

  }
  
  exitflag = work->linsys_solver->update_sigma_vec(work->linsys_solver, work->sigma_vec);
