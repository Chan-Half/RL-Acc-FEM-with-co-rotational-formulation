...
c_int adapt_sigma(OSQPWorkspace *work) {
  c_int   exitflag; // Exitflag
  c_float rho_new;  // New rho value

  exitflag = 0;     // Initialize exitflag to 0

  switch (work->settings->adaptive_sigma) {
  case RLQP_ADAPTIVE_SIGMA_STANDARD:
    // Compute new rho
    rho_new = compute_rho_estimate(work);
    ...
    break;
  case RLQP_ADAPTIVE_SIGMA_SCALAR_POLICY:
# ifdef PRINTING
    c_eprint("Scalar policy not yet implement (but will be soon!) (%s:%d)", __FILE__, __LINE__);
# endif
    exitflag = 1;
    break;
  case RLQP_ADAPTIVE_SIGMA_VECTOR_POLICY:
    exitflag = rl_policy_compute_vec_sigma(work);
    if (exitflag == 0)
      // exitflag = work->linsys_solver->update_rho_vec(work->linsys_solver, work->rho_vec);
      exitflag = work->linsys_solver->update_sigma_vec(work->linsys_solver, work->sigma_vec);
    break;
  case RLQP_ADAPTIVE_SIGMA_DISABLE:
    break;
  }
...
void set_rho_vec(OSQPWorkspace *work) {
  ...
  for (j=0; j < work->data->n; j++) {
      work->sigma_vec[j] = SIGMA;
  }
}

void set_sigma_vec(OSQPWorkspace *work) {
  c_int j;
...
static void compute_rhs(OSQPWorkspace *work) {
  ...
  for (i = 0; i < work->data->n; i++) {
    ...
    work->xz_tilde[i] = work->sigma_vec[i] * work->x_prev[i] -
                        work->data->q[i];
  }
...
c_float compute_dua_res(OSQPWorkspace *work, c_float *x, c_float *y) {
  ...
  // 本处是因为新的矩阵不对称，也不是使用上三角，所以去除了该部分的计算
...
c_int validate_linsys_solver(c_int linsys_solver) {
  //c_eprint("linsys_solver %d, SPSLU_SOLVER = %d, QDLDL_SOLVER = %d", linsys_solver,SPSLU_SOLVER, QDLDL_SOLVER);
  if ((linsys_solver != QDLDL_SOLVER) && (linsys_solver != SPSLU_SOLVER) &&
