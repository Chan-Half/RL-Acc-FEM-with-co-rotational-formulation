...
def linsys_solver_str_to_int(settings):
        ...
        elif linsys_solver_str == 'spslu':
            settings['linsys_solver'] = _rlqp.constant('SPSLU_SOLVER')
...
def adaptive_sigma_to_int(settings):
    adaptive_sigma = settings.pop('adaptive_sigma', '')
    if isinstance(adaptive_sigma, bool):
        if adaptive_sigma:
            settings['adaptive_sigma'] = _rlqp.constant('ADAPTIVE_SIGMA_STANDARD')
        else:
            settings['adaptive_sigma'] = _rlqp.constant('ADAPTIVE_SIGMA_DISABLE')
    elif isinstance(adaptive_sigma, str):
        adaptive_sigma = adaptive_sigma.lower()
        if adaptive_sigma == 'disable':
            settings['adaptive_sigma'] = _rlqp.constant('ADAPTIVE_SIGMA_DISABLE')
        elif adaptive_sigma == 'standard' or adaptive_sigma == '':
            settings['adaptive_sigma'] = _rlqp.constant('ADAPTIVE_SIGMA_STANDARD')
        elif adaptive_sigma == 'scalar_policy':
            settings['adaptive_sigma'] = _rlqp.constant('ADAPTIVE_SIGMA_SCALAR_POLICY')
        elif adaptive_sigma == 'vector_policy':
            settings['adaptive_sigma'] = _rlqp.constant('ADAPTIVE_SIGMA_VECTOR_POLICY')
        else:
            raise ValueError("Unrecognized adaptive_sigma setting")
    else:
        raise TypeError('Setting adaptive_sigma is required to be a boolean or a string')
    return settings
...
def prepare_data(P=None, q=None, A=None, l=None, u=None, **settings):
        ...
        settings = adaptive_sigma_to_int(settings)
