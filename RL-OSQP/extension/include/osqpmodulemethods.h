...
static PyObject *OSQP_constant(OSQP *self, PyObject *args) {
    ...
    if(!strcmp(constant_name, "SPSLU_SOLVER")){
		return Py_BuildValue("i", SPSLU_SOLVER);
	}
    ...
    // Adaptive sigma
    if (!strcmp(constant_name, "ADAPTIVE_SIGMA_DISABLE"))
        return Py_BuildValue("i", RLQP_ADAPTIVE_SIGMA_DISABLE);
    if (!strcmp(constant_name, "ADAPTIVE_SIGMA_STANDARD"))
        return Py_BuildValue("i", RLQP_ADAPTIVE_SIGMA_STANDARD);
    if (!strcmp(constant_name, "ADAPTIVE_SIGMA_SCALAR_POLICY"))
        return Py_BuildValue("i", RLQP_ADAPTIVE_SIGMA_SCALAR_POLICY);
    if (!strcmp(constant_name, "ADAPTIVE_SIGMA_VECTOR_POLICY"))
        return Py_BuildValue("i", RLQP_ADAPTIVE_SIGMA_VECTOR_POLICY);
