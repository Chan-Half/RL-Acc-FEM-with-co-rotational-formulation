...
static PyObject * OSQP_setup(OSQP *self, PyObject *args, PyObject *kwargs) {
    ...
    PyObject *adaptive_sigma_policy_obj = NULL;
    PyObject *adaptive_sigma_policy_bytes = NULL;
    static char *kwlist[] = {"dims",                     // nvars and ncons
                             "Px", "Pi", "Pp", "q",      // Cost function
                             "Ax", "Ai", "Ap", "l", "u", // Constraints
                             "scaling",
                             "adaptive_rho", "adaptive_sigma", "adaptive_rho_interval",
                             "adaptive_rho_tolerance", "adaptive_rho_fraction",
                             "adaptive_rho_policy", "adaptive_sigma_policy",
...
// Same with adaptive_rho now.
...
#ifdef DFLOAT
    static char * argparse_string = "(LL)O!O!O!O!O!O!O!O!O!|LiiLffOOffLffffffiLLLLLLf";
#else
    static char * argparse_string = "(LL)O!O!O!O!O!O!O!O!O!|LiiLddOOddLddddddiLLLLLLd";
#endif

#else

#ifdef DFLOAT
    static char * argparse_string = "(ii)O!O!O!O!O!O!O!O!O!|iiiiffOOffiffffffiiiiiiif";
#else
    static char * argparse_string = "(ii)O!O!O!O!O!O!O!O!O!|iiiiddOOddiddddddiiiiiiid";
#endif
    ...
    if (adaptive_sigma_policy_obj == Py_None || adaptive_sigma_policy_obj == NULL)
        adaptive_sigma_policy_bytes = NULL;
    else if (!PyUnicode_FSConverter(adaptive_sigma_policy_obj, &adaptive_sigma_policy_bytes))
        return NULL;
    else
        settings->adaptive_sigma_policy = PyBytes_AsString(adaptive_sigma_policy_bytes);
    ...
    Py_XDECREF(adaptive_sigma_policy_bytes);
...
static PyObject * OSQP_get_dx(OSQP *self, void* context) {
    if (!self->workspace) {
        PyErr_SetString(PyExc_ValueError, "Workspace not initialized!");
        return NULL;
    }
    npy_intp n = (npy_intp)self->workspace->data->n;
    PyObject *delta_x = (PyObject *)PyArrayFromCArray(self->workspace->delta_x, &n);
    return delta_x;
}

static PyObject * OSQP_get_Px(OSQP *self, void* context) {
    if (!self->workspace) {
        PyErr_SetString(PyExc_ValueError, "Workspace not initialized!");
        return NULL;
    }
    npy_intp n = (npy_intp)self->workspace->data->n;
    PyObject *Px = (PyObject *)PyArrayFromCArray(self->workspace->Px, &n);
    return Px;
}

static PyObject * OSQP_get_Aty(OSQP *self, void* context) {
    if (!self->workspace) {
        PyErr_SetString(PyExc_ValueError, "Workspace not initialized!");
        return NULL;
    }
    npy_intp n = (npy_intp)self->workspace->data->n;
    PyObject *Aty = (PyObject *)PyArrayFromCArray(self->workspace->Aty, &n);
    return Aty;
}

static PyObject * OSQP_get_q(OSQP *self, void* context) {
    if (!self->workspace) {
        PyErr_SetString(PyExc_ValueError, "Workspace not initialized!");
        return NULL;
    }
    npy_intp n = (npy_intp)self->workspace->data->n;
    PyObject *q = (PyObject *)PyArrayFromCArray(self->workspace->data->q, &n);
    return q;
}
...
static PyObject * OSQP_get_sigma_vec(OSQP *self, void* context) {
    if (!self->workspace) {
        PyErr_SetString(PyExc_ValueError, "Workspace not initialized!");
        return NULL;
    }
    npy_intp n = (npy_intp)self->workspace->data->n;
    PyObject *sigma_vec = (PyObject *)PyArrayFromCArray(self->workspace->sigma_vec, &n);
    return sigma_vec;
}
...
static int OSQP_set_sigma_vec(OSQP *self, PyObject *value, void* context) {
    ...
    if (!PyArray_Check(value)) {
        PyErr_SetString(PyExc_ValueError, "sigma vec must be a numpy array");
        return -1;
    }
    ...
    PyArrayObject *v_cont = get_contiguous((PyArrayObject*)value, float_type);
    int result = -1;
    if (pyarray_vector_size(v_cont) != n) {
        PyErr_SetString(PyExc_ValueError, "sigma vec size mismatch");
        ...
        c_float *v_arr = (c_float*)PyArray_DATA(v_cont);
        osqp_update_sigma_vec(self->workspace, v_arr);
        result = 0;
    }
    Py_DECREF(v_cont);
    return result;
...
static PyGetSetDef RLQP_getset[] = {
    ...
    { "sigma_vec", (getter)OSQP_get_sigma_vec, (setter)OSQP_set_sigma_vec, PyDoc_STR("Access to the internal sigma vector"), NULL },
    { "dx", (getter)OSQP_get_dx, NULL, PyDoc_STR("Get the current value of dx"), NULL },
    { "Px", (getter)OSQP_get_Px, NULL, PyDoc_STR("Get the current value of Px"), NULL },
    { "Aty", (getter)OSQP_get_Aty, NULL, PyDoc_STR("Get the current value of Aty"), NULL },
    { "q", (getter)OSQP_get_q, NULL, PyDoc_STR("Get the current value of x"), NULL },
