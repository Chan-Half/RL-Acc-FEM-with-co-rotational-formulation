...
def ForceSensor(shared_force):
    global force
    global torque
    Sensor().start()
    # s.run()
    while True:
        shared_force['force'] = np.array(force) - np.array([ 0.18983492,-1.04810786,-1.02710545])
        shared_force['torque'] = np.array(torque) - np.array( [-0.00090644, 0.01031971, 0.01192025])
        print('force:', force)
        print('torque:', torque, '\n')
    # Sensor().start()
    # 零点
    # force: [ 0.18983492 -1.04810786 -1.02710545]
    # torque: [-0.00090644  0.01031971  0.01192025]
    # while True:
    #     print('force:', force)
    #     print('torque:', torque, '\n')
